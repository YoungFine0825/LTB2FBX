...
*/
	...
	typedef int BOOL;
#ifdef __APPLE
	typedef unsigned int DWORD;//by yangfan Mac平台需要将DWORD定义为uint类型
#else
	typedef unsigned long DWORD;//by yangfan windows平台需要将DWORD定义为ulong类型
#endif
...
*/
	struct HPOLY
	{
        HPOLY(uint32 nWorldIndex, uint32 nPolyIndex) : m_nPolyIndex(nPolyIndex), m_nWorldIndex(nWorldIndex) {}
        HPOLY() : m_nPolyIndex(0xFFFFFFFF), m_nWorldIndex(0xFFFFFFFF) {}
