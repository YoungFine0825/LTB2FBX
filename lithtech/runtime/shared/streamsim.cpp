...
public:
	...
	LTRESULT	SeekTo(uint32 offset)
	{
		...
		if( SetFilePointer( m_hFile, offset, 0, 0 ) != ( uint32 )-1 )
		{
			return LT_OK;
		}
		else
	...
	LTRESULT	GetPos(uint32 *offset)
	{
		...
		dwOffset = SetFilePointer( m_hFile, 0, 0, 1 );
...
ILTStream* streamsim_OpenWinFileHandle( const char *pszFilename, uint32 dwDesiredAccess )
{
	...
	hFile = CreateFile( (LPCWSTR)pszFilename, dwDesiredAccess, FILE_SHARE_READ, LTNULL, OPEN_ALWAYS, 0, LTNULL );
